...
def train():
    ...
    #build, load,  the net 
    t_net = build_ssd('train','resnet50',size = cfg['min_dim'],cfg = cfg)
    supervisor_path = './work_dir/SSD300_VOC_FPN_Giou/ssd_.pth'
    t_net.load_state_dict(torch.load(supervisor_path))
    for param in t_net.parameters():
        print(param)
        param.requires_grad = False
        
    s_net = build_ssd('train','resnet18',size = cfg['min_dim'],cfg = cfg)
    ...
    distill_params = [{'params': s_net.parameters()}, {'params': d_net.Connectors.parameters()}]
    '''
    ...
    if args.resume:
        ...
        s_net.load_state_dict(torch.load(args.resume))
    ...
    #optimizer
    optimizer = optim.SGD(distill_params, lr=args.lr, momentum=args.momentum,
    ...
    for epoch in range(max_epoch):
        for ii, batch_iterator in tqdm(enumerate(data_loader)):
            ...
            loss_l=losses[0]
            loss_c=losses[1]
            loss = loss_l + loss_c + loss_distill.sum() / batch_size * 1e-5
            a=loss.item()
    ...
    torch.save(net.state_dict(),args.work_dir+cfg['work_name']+'/ssd'+'_.pth')
