...
class SSD(nn.Module):
    ...
    def forward(self, x, phase):
        ...
        print("build_ssdd output",output)
        return output
...
def build_ssd(phase, model, size=300, cfg=None):
    ...
    if model=='resnet50':
        ...
        head = SSDHead(num_classes = cfg['num_classes'],in_channels =[512,1024,2048,512,256,256],aspect_ratios = cfg['aspect_ratios'])
    else:
        base = Backbone(model,[6,7,8,9,10,11])
        neck=None
        #neck = Neck(in_channels = [128,256,512,256,256], out_channels = cfg['neck_out'])
        head = SSDHead(num_classes = cfg['num_classes'],in_channels =[128,256,512,256,256],aspect_ratios = cfg['aspect_ratios'])
